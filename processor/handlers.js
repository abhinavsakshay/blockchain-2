...
const createAsset = (asset, owner, state) => {
   const address = getAssetAddress(asset.patientName)
  console.log("++++++++",asset,"**********");
  return state.get([address])
    .then(entries => {
      ...
      return state.set({
        [address]: encode({patientName: asset.patientName,patientAge:asset.patientAge , owner})
      })
    })
}
...
class JSONHandler extends TransactionHandler {
  ...
  apply (txn, state) {
    ...
    if (action === 'createPatient') return createAsset(asset, signer, state)

    return Promise.resolve().then(() => {
      throw new InvalidTransaction(
        'Action must be "createPatient"'
      )
    })
  }
}
